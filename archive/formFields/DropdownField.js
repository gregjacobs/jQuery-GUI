...
ui.formFields.DropdownField = Class.extend( ui.formFields.WrappedInputField, {
	...
	onRender : function( container ) {
		...
		*/
		...
		var dropdownMarkup = Jux.Util.tmpl( dropdownRenderTpl, {
			...
			optionStyles  : ( option && option.style ) ? Jux.Css.hashToString( option.style ) : ""
		} );
		...
		var onDropdownClickDelegate = this.onDropdownClick.createDelegate( this );
	...
	redrawOptions : function() {
		if( this.rendered ) {
			...
			for( i = 0; i < numOptions; i++ ) {
				...
				optionsMarkup += Jux.Util.tmpl( optionsMenuRenderTpl, {
					anchorClass : ( option.cls || "" ) + ( ( option.value === currentFieldValue ) ? ' selected' : '' ),
					anchorStyle : ( option.style ) ? Jux.Css.hashToString( option.style ) : '', 
					text : option.text  
				} );
			}
	...
	setValue : function( value ) {
		...
		if( option ) {
			...
			if( this.rendered ) {
				...
				var $div = jQuery( '<div class="' + option.cls + '" style="' + ( option.style ? Jux.Css.hashToString( option.style ) : '' ) + '">' + option.text + '</div>' );
