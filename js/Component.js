...
function( jQuery, _, Class, UI, Observable, ComponentManager, Css, Html, Mask, Animation, Plugin, LoDashTpl ) {
	...
	var Component = Class.extend( Observable, { 
		...
		/**
		 * @protected
		 * @cfg {String/String[]/Function/ui.util.LoDashTpl} renderTpl
		 * 
		 * The Lo-Dash template to use to render the Component's **internal** structure. This is used for Component subclasses to
		 * generate the markup that will make up the Component's structure that does not change. This is opposed to the {@link #tpl}
		 * config, which if present, is used to update the component's markup via {@link #update}.
		 * 
		 ...
		 *         '<div id="<%= elId %>-titlebar">',  // `elId` var is automatically provided to {@link #renderTplData}, from the {@link #elId} config - See {@link #renderTplData}
		 *             '<%= title %>',                 // `title` var would need to be provided to {@link #renderTplData} by subclass
		 *         '</div>',
		 ...
		 * 
		 * This config can be a string, an array of strings, a compiled Lo-Dash template function, or a {@link ui.util.LoDashTpl} instance. 
		 * When the Component is rendered, it will be turned into a {@link ui.util.LoDashTpl LoDashTpl} instance.
		 */
		 ...
		 * 
		 * The Lo-Dash template to use as the HTML template of the Component. 
		 * 
		 ...
		 * 
		 * This can be a string, an array of strings, a compiled Lo-Dash template function, or a {@link ui.util.LoDashTpl} instance. 
		 * When the Component is rendered, it will be turned into a {@link ui.util.LoDashTpl LoDashTpl} instance.
		 * 
		...
		render : function( containerEl, options ) {
			...
			if( this.rendered ) {
				...
				var renderTplMarkup = "";
				if( this.renderTpl ) {
					renderTplMarkup = (new LoDashTpl( this.renderTpl )).apply( this.getRenderTplData() );
