...
tests.unit.ui.formFields.add( new Ext.test.Suite( {
	...
	items : [
		...
		/*
		 * Test setting the 'inputName' config
		 */
		{
			name : "Test setting the 'inputName' config",
			
			
			// TODO: Is this correct? Doesn't the "for" attribute need to relate to an ID?
			
			"The 'for' of the label and the 'name' of the field should match the 'inputName' when set" : function() {
				var textField = new ui.formFields.TextField( {
				...
				Y.Assert.areEqual( $label.length, 1, "there should be exactly one label" );
				Y.Assert.areEqual( $input.length, 1, "there should be exactly one input" );
				Y.Assert.areEqual( $label.attr( 'for' ), 'foo', "the label's 'for' attribute should match the 'inputName'" );
				Y.Assert.areEqual( $input.attr( 'name' ), 'foo', "the input's 'name' attribute should match the 'inputName'" );
