...
tests.unit.ui.layout.add( new Ext.test.Suite( {
	...
	items : [
		 ...
		 */
		{
			name : "Test setActiveItem()",
			
			"setActiveItem() should accept a number, or direct component reference to set the active item" : function() {		
		 ...
		 */
		{
			name : "Test getActiveItem()",
			
			"getActiveItem() should work with no child items" : function() {
				var container = new ui.Container( {
					layout : 'cards',
				...
				var layout = container.getLayout();
				Y.Assert.isInstanceOf( ui.layout.CardsLayout, layout, "initial condition failed. Container should have had CardsLayout." );
			...
			"getActiveItem() should work with one child item, and the `activeItem` config set to 0" : function() {
			...
			"getActiveItem() should work with one child item, and the `activeItem` config set to 1 (which should be the *second* component)" : function() {
				var cmp = new ui.Component();
				var container = new ui.Container( {
					layout : {
						type : 'cards',
			...
			"getActiveItem() should work with one child item, and the `activeItem` config set to the reference of the component" : function() {
				var cmp = new ui.Component();
				var container = new ui.Container( {
					layout : {
						type : 'cards',
		...
		/*
		 * Test getActiveItemIndex()
		 */
		{
			name : "Test getActiveItemIndex",
			
			
			"getActiveItemIndex() should work with no child items, and a number for the 'activeItem' config" : function() {
				var container = new ui.Container( {
					layout : {
						type : 'cards',
			...
			"getActiveItemIndex() should work with one child item, and an `activeItem` config that is out of bounds of the number of children" : function() {
				var container = new ui.Container( {
					layout : {
						type : 'cards',
			...
			"getActiveItemIndex() should work with no child items, and a component for the `activeItem` config" : function() {
				var container = new ui.Container( {
					layout : {
						type : 'cards',
