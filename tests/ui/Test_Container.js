	...
	items : [
		 ...
		 */
		{
			...
			test_insert : function() {
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ component1, component2, component3 ], container.getItems(), /*deep*/ false ), "initial assert" );
				
				// move component3 from the end to the beginning
				container.insert( component3, 0 );
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ component3, component1, component2 ], container.getItems(), /*deep*/ false ), "new ordering is incorrect. component3 should have been moved to the first slot" );
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ component3, component2, component1 ], container.getItems(), /*deep*/ false ), "new ordering is incorrect. component1 should have been moved to last slot" );
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ component2, component3, component1 ], container.getItems(), /*deep*/ false ), "new ordering is incorrect. component2 should have been moved to first slot" );
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ component3, component1, component2 ], container.getItems(), /*deep*/ false ), "new ordering is incorrect. component2 should have been moved to last slot" );
				Y.Assert.areEqual( container, component2.getParentContainer(), "error: component2 no longer has the container as its parent, when it originally started in that container." ); 
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ component1 ], container.getItems(), /*deep*/ false ), "new ordering is incorrect. component1 should have been inserted into the first slot with negative position arg" );
				
				container.insert( component2, -9999999 );
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ component2, component1 ], container.getItems(), /*deep*/ false ), "new ordering is incorrect. component2 should have been inserted into the first slot with negative position arg" );
				
				container.insert( component3, 9999999 );
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ component2, component1, component3 ], container.getItems(), /*deep*/ false ), "new ordering is incorrect. component3 should have been inserted into the 3rd slot with out of bounds positive position arg" );
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ component ], container.getItems(), /*deep*/ false ), "component should have been inserted into the first slot with position arg one more than the number of items" );
		 ...
		 */
		{
			...
			test_remove : function() {
				...
				container.remove( c2 );
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ c1, c3, c4 ], container.getItems(), /*deep*/ false ), "2nd component not removed correctly when removing single item" );
				
				container.remove( c4 );
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ c1, c3 ], container.getItems(), /*deep*/ false ), "last component not removed correctly when removing single item" );
				
				container.remove( c1 );
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ c3 ], container.getItems(), /*deep*/ false ), "first component not removed correctly when removing single item" );
				
				container.remove( c3 );
				Y.Assert.isTrue( Kevlar.Object.isEqual( [], container.getItems(), /*deep*/ false ), "last component left not removed correctly when removing single item" );
				...
				var childCmps = container.getItems();
				var c1 = childCmps[ 0 ], c2 = childCmps[ 1 ], c3 = childCmps[ 2 ], c4 = childCmps[ 2 ];
				
				container.remove( [ c1, c3 ] );
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ c2, c4 ], container.getItems(), /*deep*/ false ), "multiple components not removed correctly" );
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ component1, component2 ], removedComponents ), "The removed components returned by remove() does not reflect the ones actually removed." );
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( [], removedComponents ), "remove() should have returned an empty array for trying to remove components that weren't in the container" );
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( [ component1, component2 ], removedComponents ), "remove() should have returned only the two components that were actually in the container" );
		 ...
		 */
		{
			...
			test_getLayout : function() {
				...
				Y.Assert.isInstanceOf( ui.layouts.ContainerLayout, layout, "A ContainerLayout should have been created on the fly from getLayout() on a Container with no layout" );
				...
				Y.Assert.isInstanceOf( ui.layouts.ColumnsLayout, container.getLayout(), "A ColumnsLayout should have been returned from getLayout(), as that is what was configured for the Container (by its string type name)" );
				...
				Y.Assert.isInstanceOf( ui.layouts.ColumnsLayout, container.getLayout(), "A ColumnsLayout should have been returned from getLayout(), as that is what was configured for the Container (by its type name in an object)" );
				
				// Test on a Container with a given instantiated layout
				var container = new ui.Container( { layout: new ui.layouts.ColumnsLayout() } );
				Y.Assert.isInstanceOf( ui.layouts.ColumnsLayout, container.getLayout(), "A ColumnsLayout should have been returned from getLayout(), as that is what was configured for the Container (by an instantiated layout)" );
			}
		},
		 ...
		 */
		{
			...
			test_setLayout : function() {
				// Test setting a layout with a string
				var container = new ui.Container();
				container.setLayout( 'columns' );
				Y.Assert.isInstanceOf( ui.layouts.ColumnsLayout, container.getLayout(), "Setting a layout with the string 'columns' should have created a ui.layouts.ColumnsLayout" );
				...
				// Test setting a layout with a string that doesn't have an associated Layout class. Should throw an error.
				var container = new ui.Container();
				try {
				...
				// Test setting a layout with an object with no 'type' property. Should default to a ContainerLayout.
				var container = new ui.Container();
				container.setLayout( {} );
				Y.Assert.isInstanceOf( ui.layouts.ContainerLayout, container.getLayout(), "Setting a layout with an empty object should have created a ui.layouts.ContainerLayout" );
				...
				Y.Assert.isInstanceOf( ui.layouts.ContainerLayout, container.getLayout(), "Setting a layout with an object with no type property should have created a ui.layouts.ContainerLayout" );
				...
				// Test setting a layout with an object *with* a 'type' property.
				var container = new ui.Container();
				container.setLayout( { type : 'Columns' } );
				Y.Assert.isInstanceOf( ui.layouts.ColumnsLayout, container.getLayout(), "Setting a layout with an object with type 'Columns' should have created a ui.layouts.ColumnsLayout" );
				...
				Y.Assert.isInstanceOf( ui.layouts.ColumnsLayout, container.getLayout(), "Setting a layout with an object with type 'Columns' should have created a ui.layouts.ColumnsLayout (with a custom property)" );
				Y.Assert.areSame( container.getLayout().customProp, 1, "setLayout() didn't seem to create the Layout with the config object (customProp wasn't applied)" );
				...
				// Test setting a layout to an instantiated ui.layouts.AbstractLayout subclass
				var container = new ui.Container();
				var myLayout = new ui.layouts.ColumnsLayout();
				...
				// Test running setLayout() without an argument. An error should be thrown.
				var container = new ui.Container();
				try {
					container.setLayout();
				...
				// Test that setting to a second layout removes the Container reference from the first layout (sets it to null)
				var container = new ui.Container();
				var layout = new ui.layouts.ColumnsLayout();
				var layout2 = new ui.layouts.ContainerLayout();
				Y.Assert.isNull( layout.getContainer(), "initial condition failed. the layout should not yet have a Container reference. should be null" );
				container.setLayout( layout );
				Y.Assert.areEqual( container, layout.getContainer(), "layout's Container reference was not set to the Container when given an instantiated Layout." );
				container.setLayout( layout2 );
				Y.Assert.areEqual( container, layout2.getContainer(), "layout2's Container reference was not set to the Container when given an instantiated Layout." );
				Y.Assert.isNull( layout.getContainer(), "the original layout (the one overwritten in the Container) did not get its Container reference set back to null" );
		 ...
		 */
		{
			...
			test_setData : function() {
				var TestDataControl = Kevlar.extend( ui.Component, [ ui.DataControl ], {
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( dataObj, container.getData() ), "The data retrieved from the container does not match the expected data" );
				...
				var TestDataContainer = Kevlar.extend( ui.Container, [ ui.DataControl ], {
					setData : function( data ) { this.data = data; },
					getData : function() { return this.data; } 
				} );
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( expectedData, container.getData() ), "The data retrieved from the container does not match the expected data" );
		 ...
		 */
		{
			...
			test_getData : function() {
				...
				Y.Assert.isTrue( Kevlar.Object.isEqual( { key1: 1, key2: 2, key3: 3, key4: 4, key5: 5 }, container.getData() ), "The data retrieved from the container does not match the expected data" );
				...
				var expectedData = { key1: 1, key2: 2, key3: 3, key4: 4, key5: 5, key6: 6, key7: 7 };
				Y.Assert.isTrue( Kevlar.Object.isEqual( expectedData, container.getData() ), "The data retrieved from the container does not match the expected data with nested DataControl Containers" );
