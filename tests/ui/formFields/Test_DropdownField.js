Ext.test.Session.addTest( 'ui.formFields', {
    ...
    name: 'Kevlar.ui.formFields.DropdownField',
	...
	// "enum" for the two checks that each test does
	...
	"The initial value for the field with no 'value' config should be the value of the first option" : function() {
		for( var testNum = 0; testNum <= 1; testNum++ ) {
			// Test with no initial value config
			var field = new ui.formFields.DropdownField( {
				renderTo : ( testNum === 0 ) ? undefined : document.body,  // render to the document.body on the second loop iteration
				options : [
